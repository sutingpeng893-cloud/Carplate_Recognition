...
@router.post("/api/chatbox/speech/stream", response_model=None)
async def chatbox_speech_stream(request: Request):
    payload, error_response = await read_json_object(request)
    if error_response is not None:
        return error_response
...
async def handle_control_message(session: AudioSession, text: str) -> None:
    ...
    if event_type == "start":
        ...
        session.stream_speech_audio = bool(payload.get("streamSpeechAudio")) and speech_streaming_supported()
