...
def resample_wav_bytes(wav_bytes: bytes, target_rate: int) -> bytes:
    samples, source_rate = wav_bytes_to_float_samples(wav_bytes)
    if source_rate != target_rate:
        samples = resample_linear(samples, source_rate, target_rate)
    return pcm_float_to_wav_bytes(samples, target_rate)


def wav_bytes_to_pcm16le_bytes(wav_bytes: bytes, target_rate: int) -> bytes:
    samples, source_rate = wav_bytes_to_float_samples(wav_bytes)
    if source_rate != target_rate:
        samples = resample_linear(samples, source_rate, target_rate)
    return float_samples_to_pcm16le_bytes(samples)
